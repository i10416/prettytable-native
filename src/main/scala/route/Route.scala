...
final case class UnsupportedExt(ext:String) extends ShowCSVError {
  override def msg: String = s"User error: $ext is not supported. Supported Extensions: ${SupportedExts.all}"
}
...
class Route(generateTableUsecase: GenerateTableUsecase,generateJsonTableUsecase:GenerateJsonTableUsecase) {
  def genTable(noHeader:Boolean,cmd: ShowCmdOptions): Either[ShowCSVError, Table] = {
    val path = os.pwd / cmd.fileName
    if (os.isFile(path)) {
      path.ext match {
                  case SupportedExts.csv => generateTableUsecase.execute(path,hasHeader = !noHeader)
